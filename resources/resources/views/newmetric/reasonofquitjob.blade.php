...
<div class="container" style="padding: 30px;">
	<div class="row">
    	...
    	?>
		<form method="GET" action="/sub-metrics/reason-of-quit-job" class="date-form">
			<div class="row">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Firmanızın İşten Ayrılış Nedenleri</h3>
					</div>
				</div>
				@if($firmadata->count()==0)
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							@if($verivarmi!=0)
							Seçilen tarih aralığında firmanızdan işten ayrılan personel bulunmadığından karşılaştırma yapılamamaktadır.
							@else
				...
				<div class="card-body" style="<?php if($firmadata->count()==0) echo 'display: none;' ?>">
					...
					<!--begin::Chart-->
					<div id="chartdiv" style="height: 650px;margin-bottom: 20px;margin-top: 50px;"></div>
					<!--end::Chart-->
				</div>
			</div>
			<!--end::Card-->
		...
		<div class="col-lg-12">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Sektör İşten Ayrılış Nedenleri</h3>
					</div>
				</div>
					...
					for ($i=0; $i <$firmadata->count() ; $i++) {
						...
						if ($indis!=-1) {
							...
							if ($firma_yuzdesi<=$max AND $firma_yuzdesi>=$min) {
								$nedenler[$i]=$firmadata[$i]['istencikmanedeni']." NEDENİYLE İŞTEN AYRILMA DEĞERLENDİRMESİ : ";
								$degerlendirme_mesaji[$i]="Değerlendirme oranınız sektör ortalamasının içerisinde olup Birim maliyet oranınız sektör ortalamasına göre nominal değerdedir."; ?>
								<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #424a52;border-color: #424a52">
									...
									<div class="alert-text" style="color: #fff;">
										<strong>{{$firmadata[$i]['istencikmanedeni']}} NEDENİYLE İŞTEN AYRILMA DEĞERLENDİRMESİ : </strong>Değerlendirme oranınız sektör ortalamasının içerisinde olup Birim maliyet oranınız sektör ortalamasına göre nominal değerdedir.
									</div>
								</div><?php
							}elseif ($firma_yuzdesi<$min) {
								$nedenler[$i]=$firmadata[$i]['istencikmanedeni']." NEDENİYLE İŞTEN AYRILMA DEĞERLENDİRMESİ : ";
								$degerlendirme_mesaji[$i]="Değerlendirme oranınız sektör ortalama değerinin altında olup ,işten ayrılış oranınız durumunuz olumlu yönde seyretmektedir."; ?>
								<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #2a6ab2;border-color: #2a6ab2">
									...
									<div class="alert-text" style="color: #fff;">
										<strong>{{$firmadata[$i]['istencikmanedeni']}} NEDENİYLE İŞTEN AYRILMA DEĞERLENDİRMESİ : </strong>Değerlendirme oranınız sektör ortalama değerinin altında olup ,işten ayrılış oranınız durumunuz olumlu yönde seyretmektedir.
									</div>
								</div><?php
								...
								<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #e74332;border-color: #e74332">
									...
									<div class="alert-text" style="color: #fff;">
										<strong>{{$firmadata[$i]['istencikmanedeni']}} NEDENİYLE İŞTEN AYRILMA DEĞERLENDİRMESİ : </strong>Değerlendirme oranınız sektör ortalama değerinin üzerinde olup, artış durumu neticesinde iş gücü kayıpları,iş kazası ,birim maliyet  ve eğitim oranlarında artış görülmesi muhtemeldir.
									</div>
								</div><?php
...
<script type="text/javascript">
	...
	var nedenler=new Array();
	var degerlendirme_mesaji=new Array();
	var job_yuzde=0;
	var arrayfirma=<?php echo json_encode($firmadata); ?>;
	if (arrayfirma.length>0) {
		nedenler=<?php if(isset($nedenler)){echo json_encode($nedenler);}else{echo json_encode(['Bilinmeyen']);} ?>;
		...
		for (var i =0 ; i < arrayfirma.length; i++) {
			firma_job[i] = {"baslik":arrayfirma[i]['istencikmanedeni'],"yuzde":arrayfirma[i]['toplamneden']};
...
<script type="text/javascript">
	...
	for (var i =0 ; i < arraysektor.length; i++) {
		sektor_job[i] = {"baslik":arraysektor[i]['istencikmanedeni'],"yuzde":arraysektor[i]['toplamneden']};
	}
