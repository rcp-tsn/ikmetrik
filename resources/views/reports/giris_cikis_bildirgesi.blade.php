                    ...
                    <div class="card card-custom gutter-b ">
                        <div class="card-header h-auto border-0">
                            <div class="card-title py-5">
                                <h3 class="card-label">
                                    <span class="d-block text-dark font-weight-bolder">İşe Giriş Çıkış Bildirgesi</span>
                                </h3>
                            </div>
                        </div>
                        <div class="card-body">
                                ...
                                <div class="col-lg-12">
                                    <div class="card card-body border-top-1 border-top-pink">
                                        <div class="text-center">
                                            <h6 class="m-0 font-weight-semibold">İşe Giriş Çıkış Bildirgesi</h6>
                                            <p class="text-muted mb-3">SGK Ebildirge V1 sistemi üzerinden bildirgeleri çeker.</p>
                                            <button type="button" onclick="stepManager.work('giris_cikis');" class="btn btn-success start-incentive"><b><i class="icon-collaboration"></i></b> Devam Et</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table class="table table-bordered" id="kt_datatable">
                                    ...
                                    @foreach($infos as $info)
                                        <tr>
                                            ...
                                            <td>{{ $info->isim }}</td>
                                            <td>{{ $info->soyisim }}</td>
                                            <td>{{ $info->ilk_soyadi }}</td>
                                            <td>{{ $info->baba_adi }}</td>
                                            <td>{{ $info->ana_adi }}</td>
                                            <td>{{ $info->dogum_yeri }}</td>
                                            <td>{{ $info->birth_date->format('d/m/Y') }}</td>
                                            <td>{{ $info->uyruk }}</td>
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="10">Veri bulunamamıştır...</td>
                                        </tr>
                                    @endif
                                    </tbody>
                                </table>
...
@push('scripts')
    <script>
        var steps = {
            'giris_cikis': {
                url: '{!! route('declarations.giris-cikis-bildirgesi-cek') !!}',
                loginurl: '{!! route('declarations.giris-cikis-bildirgesi-post') !!}',
            },

        };

        var stepManager = {
            currentStep: 'giris_cikis',
            work: function (step) {
