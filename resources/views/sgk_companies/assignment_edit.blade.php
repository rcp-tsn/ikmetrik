            ...
            <div class="row mt-0 mt-lg-8">
                <div class="col-xl-12">
                    ...
                    <div class="card card-custom gutter-b ">
                        ...
                        <!--begin::Card body-->
                        {!! Form::model($user, ['route' => ['company_assignments.update', createHashId($user->id)], 'method' => 'PUT', 'class' => 'form', 'id' => 'ezy_standard_form']) !!}
                        <div class="card-body assignment-list-area">
                            <select id="duallistbox_demo1" name="sgk_companies[]" multiple="multiple">
                                @foreach($sgk_companies as $sgk_company)
                                    <option value="{{ $sgk_company->id }}" {{ in_array($sgk_company->id, $currentAssignments) ? 'selected' : '' }}>{{ $sgk_company->name }}</option>
                                @endforeach
                            </select>
...
@push('scripts')
<script>
    var demo1 = $('select[name="sgk_companies[]"]').bootstrapDualListbox();
