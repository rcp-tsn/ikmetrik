...
<div class="container" style="padding: 30px;">
	<div class="row">
    	...
    	?>
		<form method="GET" action="/sub-metrics/labor-loss-rate" class="date-form">
			<div class="row">
		...
		<div class="col-lg-12">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title" style="width: 100%;">
						<h3 class="card-label"style="width: 50%;">
							Firmanızın İş Gücü Kayıp Oranı : %{{number_format($firma_yuzdesi,1)}}
						</h3>
						<h3 class="card-label" style="width: 50%;text-align: right;">
							Sektör İş Gücü Kayıp Oranı :  %{{number_format($sektor_yuzdesi,1)}}
						</h3>
					</div>
				</div>
					...
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							...
							@elseif($firma_yuzdesi==0)
							 @if($eksik_gun==0 AND $planlanan_gun!=0)
							 	Seçilen tarih aralığında firmanızda eksik gün çalışan personel olmadığından karşılaştırma yapılamamaktadır.
							 @else
				...
				<div class="card-body" style="<?php if($firma_yuzdesi==0 || $sektor_yuzdesi==0) echo 'display: none;' ?>">
					...
					<span><b>Firmanızın Eksik Gün Sayısı : {{number_format($eksik_gun,0,',','.')}} Gün</b></span><br>
					<span><b>Firmanızın Planlanan Gün Sayısı : {{number_format($planlanan_gun,0,',','.')}} Gün</b></span>
					<!--begin::Chart-->
					...
					if ($firma_yuzdesi<=$max AND $firma_yuzdesi>=$min) {
						...
						$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalama değerinin üzerinde olup, artış durumu neticesinde fazla mesai,iş kazası ,turnover oranlarında artış görülmesi muhtemeldir."; ?>
						<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #e74332;border-color: #e74332">
							...
							<div class="alert-text" style="color: #fff;">
								Değerlendirme oranınız sektör ortalama değerinin üzerinde olup, artış durumu neticesinde fazla mesai,iş kazası ,turnover oranlarında artış görülmesi muhtemeldir.
							</div>
						</div><?php
...
<script type="text/javascript">
	var eksik_gun=<?php echo $eksik_gun; ?>;
	var planlanan_gun=<?php echo $planlanan_gun; ?>;
	var firma_yuzdesi=<?php echo $firma_yuzdesi; ?>;
	var sektor_eksik_gun=<?php  echo $sektor_eksik_gun; ?>;
	var sektor_planlanan_gun=<?php  echo $sektor_planlanan_gun; ?>;
	var sektor_yuzdesi=<?php  echo $sektor_yuzdesi; ?>;
		...
		chart.data = [{
		  "baslik": "Sektör Bazlı Firmanızın İş Gücü Kayıp Oranı",
		  "yuzde": firma_yuzdesi
		}, {
		  "baslik": "Sektörün Firmanıza Göre İş Gücü Kayıp Oranı",
