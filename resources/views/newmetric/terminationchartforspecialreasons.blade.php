...
<div class="container" style="padding: 30px;">
	<div class="row">
    	...
    	?>
		<form method="GET" action="/sub-metrics/termination-chart-for-special-reasons" class="date-form">
			<div class="row">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Özel Nedenlerle İşten Ayrılma Oranı : %{{number_format($firma_yuzdesi,1)}}</h3>
					</div>
				</div>
				@if($firma_evlilikten_ayrilanlar==0 && $firma_emeklilikten_ayrilanlar==0 && $firma_askerlikten_ayrilanlar==0)
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							Seçilen tarih aralığında firmanızdan özel nedenlerden dolayı ayrılan personel yoktur.
						</div>
					</div>
				@elseif($firma_yuzdesi==0)
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							Seçilen tarih aralığında firmanıza ait veri yoktur.
						</div>
					</div>
				@endif
				<div class="card-body" style="<?php if($firma_evlilikten_ayrilanlar==0 && $firma_emeklilikten_ayrilanlar==0 && $firma_askerlikten_ayrilanlar==0) echo 'display: none;' ?>">
		...
		<div class="col-lg-12" style="<?php if($sektor_evlilikten_ayrilanlar==0 && $sektor_emeklilikten_ayrilanlar==0 && $sektor_askerlikten_ayrilanlar==0) echo 'display: none;' ?>">
			<!--begin::Card-->
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Sektör Özel Nedenlerle İşten Ayrılma Oranı : %{{number_format($sektor_yuzdesi,1)}}</h3>
					</div>
				</div>
				<div class="card-body">
					<!--begin::Chart-->
					...
					<?php
						...
						if (($firma_yuzdesi<=$max AND $firma_yuzdesi>=$min) && ($firma_evlilikten_ayrilanlar!=0 || $firma_emeklilikten_ayrilanlar!=0 || $firma_askerlikten_ayrilanlar!=0)) {
							$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalamasının içerisinde olup özel nedenlerden istifa oranınız sektör ortalamasına göre nominal değerdedir."; ?>
						...
						}elseif ($firma_yuzdesi<$min && ($firma_evlilikten_ayrilanlar!=0 || $firma_emeklilikten_ayrilanlar!=0 || $firma_askerlikten_ayrilanlar!=0)) {
							$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalama değerinin altında olup ,özel nedenlerden istifa  oranınız olumlu yönde seyretmektedir."; ?>
							...
							$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalama değerinin üzerinde olup, artış durumu neticesinde iş gücü kayıpları,iş kazası ,birim maliyet  ve eğitim oranlarında artış görülmesi muhtemeldir."; ?>
		    ...
		    doc.content.push({
		  	...
		  	doc.content.push({
		  		text:[{
			      	...
			      	text: '<?php if (isset($degerlendirme_mesaji)) {echo $degerlendirme_mesaji;}else{echo "Değerlendirme yapılamamaktadır.";}  ?>',
			      	fontSize: 12,
			      	bold: false,
			    }],
		  		margin: [0,10],
