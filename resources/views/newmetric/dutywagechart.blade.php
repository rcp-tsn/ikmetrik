...
<div class="container" style="padding: 30px;">
	<div class="row">
		<div class="col-lg-12">
			<h2>Görev Ücret Grafiği Nedir?</h2>
			<div class="metric-image-div">
				<img src="/pictures/28.jpg" style="width: 100%;height: auto;">
			</div>
			<div class="metric-define-div">
				<p style="text-align: justify;">
					İşletmede çalışan bireylerin, bildirge üzerinden tanımlanan görev/meslek kodu üzerinde ücret ilişkisini, meslek bazında dağılımlarını ifade eder. Mevcut ücret/görev ilişkisinin, özellikle yetkinlik çerçevesinden tekrar analiz edilmesini sağlar. Bildirgeler üzerinden tanımlanan görevlerin, ücret ile ilişkisini tanımlamaktadır.<br><br>
					<span class="formulgoster" id="formulgoster">Formülü Göster <i class="fas fa-caret-down"></i></span><br>
					<span class="formul"><b>Hesaplama Formülü :</b> Görev bazında ücret dağılım grafiği</span>
					<br>
				</p>
			</div>
		</div>
    	...
    	?>
		<form method="GET" action="/sub-metrics/duty-wage-chart" class="date-form">
			<div class="row">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Görev Ücret Grafiği</h3>
					</div>
				</div>
				...
				<div class="card-body" style="<?php if($firmadata->count()==0) echo 'display: none;' ?>">
					...
					<!--begin::Chart-->
					<div id="chartdiv" style="height: 500px;margin-bottom: 20px;margin-top: 50px;"></div>
					<!--end::Chart-->
				</div>
			</div>
			<!--end::Card-->
		...
		<div class="col-lg-12">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title">
						<h3 class="card-label">Sektör Görev Ücret Grafiği</h3>
					</div>
				</div>
				...
				<div class="card-body" style="<?php if($sectordata->count()==0) echo 'display: none;' ?>">
					...
					for ($i=0; $i <$firmadata->count() ; $i++) {
						...
						if ($indis!=-1) {
							$meslek_maliyet=122.5*($firmadata[$i]['toplamucret']+$firmadata[$i]['toplamikramiye'])/100;
							$firma_yuzdesi=$meslek_maliyet*100/$firma_toplam_maliyet;
							$sektor_meslek_maliyet=122.5*($sectordata[$i]['toplamucret']+$sectordata[$i]['toplamikramiye'])/100;
							$sektor_yuzdesi=+$sektor_meslek_maliyet*100/$sektor_toplam_maliyet;
							...
							if ($firma_yuzdesi<=$max AND $firma_yuzdesi>=$min) {
								$departmanlar[$i]=$firmadata[$i]['departmanadi'];
								$degerlendirme_mesaji[$i]="Değerlendirme oranınız sektör ortalamasının içerisinde olup görev ücret matriksi oranınız sektör ortalamasına göre nominal değerdedir."; ?>
								<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #424a52;border-color: #424a52;<?php if($i!=0){echo "margin-top: 10px;";} ?>">
									...
									<div class="alert-text" style="color: #fff;">
										<strong>{{$firmadata[$i]['departmanadi']}} ÜCRET DEĞERLENDİRMESİ : </strong>Değerlendirme oranınız sektör ortalamasının içerisinde olup görev ücret matriksi oranınız sektör ortalamasına göre nominal değerdedir.
									</div>
								</div><?php
							}elseif ($firma_yuzdesi<$min) {
								$departmanlar[$i]=$firmadata[$i]['departmanadi'];
								$degerlendirme_mesaji[$i]="Değerlendirme oranınız sektör ortalama değerinin altında olup ,görev ücret matriski durumunuz olumlu yönde seyretmektedir."; ?>
								<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #2a6ab2;border-color: #2a6ab2;<?php if($i!=0){echo "margin-top: 10px;";} ?>">
									...
									<div class="alert-text" style="color: #fff;">
										<strong>{{$firmadata[$i]['departmanadi']}} ÜCRET DEĞERLENDİRMESİ : </strong>Değerlendirme oranınız sektör ortalama değerinin altında olup ,görev ücret matriski durumunuz olumlu yönde seyretmektedir.
									</div>
								</div><?php
								...
								<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #e74332;border-color: #e74332;<?php if($i!=0){echo "margin-top: 10px;";} ?>">
									...
									<div class="alert-text" style="color: #fff;">
										<strong>{{$firmadata[$i]['departmanadi']}} ÜCRET DEĞERLENDİRMESİ : </strong>Değerlendirme oranınız sektör ortalama değerinin üzerinde olup, artış durumu neticesinde rakabet ve satış süreçlerinde zorluklar yaşanması muhtemeldir.
									</div>
								</div><?php
...
<script type="text/javascript">
	...
	if (arrayfirma.length!=0) {
		...
		for (var i =0 ; i < arrayfirma.length; i++) {
			job_yuzde =parseInt(arrayfirma[i]['toplamucret']+arrayfirma[i]['toplamikramiye']);
			firma_job[i] = {"baslik":arrayfirma[i]['departmanadi'],"yuzde":job_yuzde};
			//series.slices.template.tooltipText =parseFloat(job_yuzde).toFixed(2)+"₺";
		}
		...
		series.slices.template.tooltipText = "{category} Görev Maliyeti : {value}₺";
...
<script type="text/javascript">
	...
	var sektordeki_firma_sayisi = <?php echo $sektordeki_firma_sayisi; ?>;
	var arraysektor=<?php echo json_encode($sectordata); ?>;
	for (var i =0 ; i < arraysektor.length; i++) {
		sektor_job_yuzde=parseFloat((parseInt(arraysektor[i]['toplamucret']+arraysektor[i]['toplamikramiye'])*122.5/100)/sektordeki_firma_sayisi).toFixed(2);
		sektor_job[i] = {"baslik":arraysektor[i]['departmanadi'],"yuzde":sektor_job_yuzde};
	}
...
<script type="text/javascript">
		$('#savepdf').click(function() {
		  ...
		  Promise.all([
			    ...
			    if (degerlendirme_mesaji.length>0 || departmanlar.length>0) {
				    doc.content.push({
				  	...
				  	for (var i =0 ; i < degerlendirme_mesaji.length; i++) {
					  	doc.content.push({
					  		text:[{
						      	text: '- '+departmanlar[i]+' ÜCRET DEĞERLENDİRMESİ : ',
						      	fontSize: 12,
						      	bold: true,
						    },{
			...
			if (firma_job.length>0) {
				doc.content.push({
			      text: "Görev Ücret Matrisi",
			      fontSize: 14,
			      bold: true,
			      margin: [0, 40, 0, 10]
			    });
				doc.content.push({
					...
					table: {
				        ...
				        headerRows: 1,
				        widths: [ 200, 200 ],
				        header:'Görev Ücret Tablosu',
				        body: [
				        	[ { text: 'Görev Adı', bold: true }, { text: 'Görev Ücreti', bold: true } ],
				        	<?php 
				        	foreach ($firmadata as $afirma): 
				        		echo "['".$afirma['departmanadi']."','".number_format($afirma['toplamucret']+$afirma['toplamikramiye'],2,'.',',')." ₺'],";
				        	endforeach; ?>
				        ]
				    },
				...
				doc.content.push({
			  		text:[{
				      	...
				      	text: 'Maliyeti en yüksek olan ilk on görev grafikte gösterilmiştir.',
				      	fontSize: 12,
				      	bold: false,
				    }],
