	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="metric-image-div">
				<img src="/pictures/18.jpg" style="width: 100%;height: auto;">
			</div>
			<div class="metric-define-div">
				<p style="text-align: justify;">
					İstifa, kişinin hür iradesi ile işinden ayrılması şeklinde tanımlanmaktadır. İşten ayrılma nedenleri incelenerek işten ayrılış nedenleri saptanır, ona göre önlem alınarak çalışanları tutundurmanın artırılması amaçlanır. İşten ayrılış nedeninin saptanabilmesi için“Çıkış Mülakatları”kullanılabilir. Bu aşamada çalışandan iş ortamındaki çeşitli insan kaynakları uygulamalarının değerlendirilerek işten ayrılmaya neden olan ana faktörlerin açıklanması istenir. İstifa Oranı neden önemlidir? Çalışanların yerine nitelikli işgücü bulmak veya hazırlamak zaman gerektirmektedir. Bu da iş gücü kaybına ve üretimde aksaklıklara neden olabilir. Bir yıl içerisinde istifa yolu ile işten ayrılanların oranını ifade eder.<br><br>
					<span class="formulgoster" id="formulgoster">Formülü Göster <i class="fas fa-caret-down"></i></span><br>
					<span class="formul"><b>Hesaplama Formülü :</b> Belirtilen Tarih Aralığında İstifa Edenler / Belirtilen Tarih Aralığında Toplam İşe Alınanlar</span><br>
				</p>
			</div>
		</div>
		<?php
    	...
    	?>
		<form method="GET" action="/sub-metrics/resign-rate" class="date-form">
			<div class="row">
		...
		<div class="col-lg-12">
			...
			<div class="card card-custom gutter-b">
					...
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							...
							@elseif($firma_yuzdesi==0)
								@if($firma_isten_ayrilan_sayisi==0 || $firma_calisan_sayisi==0)
									Seçilen tarih aralığında firmanıza alınan veya firmanızdan ayrılan personel olmadığından ötürü karşılaştırma yapılamamaktadır.
								@else
				...
				<div class="card-body" style="<?php if($firma_yuzdesi==0 || $sektor_yuzdesi==0) echo 'display: none;' ?>">
					...
					<span><b>İşten Ayrılan Personel Sayısı : {{number_format($firma_isten_ayrilan_sayisi,0,',','.')}} Kişi</b></span><br>
					<span><b>İşe Alınan Personel Sayısı : {{number_format($firma_calisan_sayisi,0,',','.')}} Kişi</b></span>
					<!--begin::Chart-->
					...
					if ($firma_yuzdesi<=$max AND $firma_yuzdesi>=$min) {
						$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalamasının içerisinde olup istifa oranınız sektör ortalamasına göre nominal değerdedir."; ?>
						...
						$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalama değerinin altında olup ,istifa  oranınız   olumlu yönde seyretmektedir."; ?>
