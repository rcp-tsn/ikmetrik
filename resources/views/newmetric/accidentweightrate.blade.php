...
<div class="container" style="padding: 30px;">
	<div class="row">
    	...
    	?>
		<form method="GET" action="/sub-metrics/accident-weight-rate" class="date-form">
			<div class="row">
		...
		<?php
			if ($firma_calisilan_gun_toplami==0) {
				$firma_yuzdesi= 0;
			}else{
				$firma_yuzdesi= ($firma_kayip_gun_sayisi/$firma_calisilan_gun_toplami)*1000;
			}/*
			if ($sektor_calisilan_gun_toplami==0) {
			  	$sektor_yuzdesi= 0;
			}else{
				$sektor_yuzdesi= $sektor_kayip_gun_sayisi/$sektor_calisilan_gun_toplami*1000;
			} */
		?>
		<div class="col-lg-12">
			<!--begin::Card-->
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title" style="width: 100%;">
						<h3 class="card-label"style="width: 50%;">
							Firmanızın Kaza Ağırlık Oranı : {{number_format($firma_yuzdesi,1)}}
						</h3>
						<h3 class="card-label" style="width: 50%;text-align: right;">
							Sektör Kaza Ağırlık Oranı :  {{number_format($sektor_yuzdesi,1)}}
						</h3>
					</div>
				</div>
					...
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							...
							@elseif($firma_yuzdesi==0)
								@if($firma_kayip_gun_sayisi==0 AND $firma_calisilan_gun_toplami!=0)
				...
				<div class="card-body" style="<?php if($firma_yuzdesi==0 || $sektor_yuzdesi==0) echo 'display: none;' ?>">
					...
					<span><b>Firmanızın Kayıp Gün Sayısı : {{number_format($firma_kayip_gun_sayisi,0,',','.')}} Gün</b></span><br>
...
<script type="text/javascript">
	...
	var sektor_yuzdesi=<?php  echo $sektor_yuzdesi; ?>;
		...
		chart.data = [{
		  "baslik": "Sektör Bazlı Firmanızın Kaza Ağırlık Oranı",
		  "yuzde": firma_yuzdesi
		}, {
		  "baslik": "Sektörün Firmanıza Göre Kaza Ağırlık Oranı",
