...
<div class="container" style="padding: 30px;">
	<div class="row">
		<div class="col-lg-12">
			<h2>Şirket İçi Nakil Oranı Nedir?</h2>
			<div class="metric-image-div">
				<img src="/pictures/7.jpg" style="width: 100%;height: auto;">
			</div>
			<div class="metric-define-div">
				<p style="text-align: justify;">
					Şirket içerisinde çalışanın yerine getirdiği değişik işlerin sayısını artırmak ve motivasyonu sağlamak amacıyla sistematik olarak çalışanı bir işten başka bir işe (bir görevden diğer bir göreve) geçirmek olarak tanımlanmaktadır. Rotasyon yapılmasının amacı, yoğun dikkat isteyen ve tekrarlanan işlerin, çalışanlar üzerindeki sıkıntı ve psikolojik yorgunluğu önlemek, verimliliği arttırmaktır.<br><br>
					<span class="formulgoster" id="formulgoster">Formülü Göster <i class="fas fa-caret-down"></i></span><br>
					<span class="formul"><b>Hesaplama Formülü :</b> Nakil Yapılan Personel Sayısı/Ortalama Personel Sayısı</span><br>
				</p>
			</div>
		</div>
		<?php
    	...
    	?>
		<form method="GET" action="/sub-metrics/transfer-rate-in-company" class="date-form">
			<div class="row">
		...
		<div class="col-lg-12">
			...
			<div class="card card-custom gutter-b">
				<div class="card-header">
					<div class="card-title" style="width: 100%;">
						<h3 class="card-label"style="width: 50%;">
							Firmanızın Şirket İçi Nakil Oranı : %{{number_format($firma_yuzdesi,1)}}
						</h3>
						<h3 class="card-label" style="width: 50%;text-align: right;">
							Sektör Şirket İçi Nakil Oranı :  %{{number_format($sektor_yuzdesi,1)}}
						</h3>
					</div>
				</div>
					...
					<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #ff7f00;border-color: #ff7f00;margin-top: 20px;">
						...
						<div class="alert-text" style="color: #fff;">
							...
							@elseif($firma_yuzdesi==0)
								@if($firma_nakil_yapilan_personel_sayisi==0 AND $firma_ortalama_kisi_sayisi!=0)
								Seçilen tarih aralığında firmanızda şirket içi nakil yapılan personel yoktur. Dolayısıyla karşılaştırma yapılamamaktadır.
								@else
				...
				<div class="card-body" style="<?php if($firma_yuzdesi==0 || $sektor_yuzdesi==0) echo 'display: none;' ?>">
					...
					<span><b>Firmanızın Nakil Yapılan Personel Sayısı : {{number_format($firma_nakil_yapilan_personel_sayisi,0,',','.')}} Kişi</b></span><br>
					<span><b>Firmanızın Ortalama Çalışan Sayısı : {{number_format($firma_ortalama_kisi_sayisi,0,',','.')}} Kişi</b></span>
					<!--begin::Chart-->
					...
					if ($firma_yuzdesi<=$max AND $firma_yuzdesi>=$min) {
						$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalamasının içerisinde olup Birim maliyet oranınız sektör ortalamasına göre nominal değerdedir."; ?>
						...
						$degerlendirme_mesaji="Değerlendirme oranınız sektör ortalama değerinin altında olup ,şirket içi rotasyon maliyet oranı durumunuz olumlu yönde seyretmektedir."; ?>
						<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #2a6ab2;border-color: #2a6ab2">
							...
							<div class="alert-text" style="color: #fff;">
								Değerlendirme oranınız sektör ortalama değerinin altında olup ,şirket içi rotasyon maliyet oranı durumunuz olumlu yönde seyretmektedir.
							</div>
						</div><?php
...
<script type="text/javascript">
	...
	var sektor_yuzdesi=<?php  echo $sektor_yuzdesi; ?>;
		...
		chart.data = [{
		  "baslik": "Sektör Bazlı Firmanızın Şirket İçi Nakil Oranı",
		  "yuzde": firma_yuzdesi
		}, {
		  "baslik": "Sektörün Firmanıza Göre Şirket İçi Nakil Oranı",
