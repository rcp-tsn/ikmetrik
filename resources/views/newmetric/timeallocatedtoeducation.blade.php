	...
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="metric-image-div">
				<img src="/pictures/17.jpg" style="width: 100%;height: auto;">
			</div>
			<div class="metric-define-div">
				<p style="text-align: justify;">
					Eğitim, bireye yeni bir davranış kazandırmak yada mevcut davranışını değiştirmek olarak tanımlanmaktadır. Eğitim şirketin organizasyonel performansının artması, çalışanların becerilerinin ve bilgilerinin kurumsal düzene uygun olarak yapılmasını sağlamaktır. İşletme içerisinde gerçekleştirilen eğitim süresinin oranını ifade eder.<br><br>
					<span class="formulgoster" id="formulgoster">Formülü Göster <i class="fas fa-caret-down"></i></span><br>
					<span class="formul"><b>Hesaplama Formülü :</b> Toplam eğitim süresi/Ortalam çalışan sayısı(ay başı +ay sonu personel sayısı/2)</span><br>
				</p>
			</div>
		</div>
			...
			<div class="card card-custom gutter-b">
				...
				<div class="card-body" style="<?php if($error_message==1) echo 'display: none;' ?>">
					...
					if ($firma_current_rate<=$max AND $firma_current_rate>=$min) {
						$degerlendirme_mesaji="Değerlendirme oranınız önceki yılların ortalama değerlerinizin içerisinde olup eğitime ayrılan süre oranınız önceki yılların ortalamasına göre nominal değerdedir.";?>
						<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #424a52;border-color: #424a52">
							...
							<div class="alert-text" style="color: #fff;">
								Değerlendirme oranınız önceki yılların ortalama değerlerinizin içerisinde olup eğitime ayrılan süre oranınız önceki yılların ortalamasına göre nominal değerdedir.
							</div>
						</div><?php
					}elseif ($firma_current_rate<$min) {
						$degerlendirme_mesaji="Değerlendirme oranınız önceki yılların ortalama değerlerinizin altında olup ,eğitime ayrılan süre oranınız olumsuz yönde seyretmektedir."; ?>
						<div class="alert alert-custom alert-outline-primary fade show mb-5" role="alert" style="background-color: #e74332;border-color: #e74332">
							...
							<div class="alert-text" style="color: #fff;">
								Değerlendirme oranınız önceki yılların ortalama değerlerinizin altında olup ,eğitime ayrılan süre oranınız olumsuz yönde seyretmektedir.
							</div>
						</div><?php

					}elseif ($firma_current_rate>$max) {
						$degerlendirme_mesaji="Değerlendirme oranınız önceki yılların ortalama değerlerinizin üzerinde olup, artış durumu neticesinde iş kazasının azalması,verimlilik,performans süreçlerinde artış görülmesi muhtemeldir."; ?>
	...
	if (firmaeducationdata!=0) {
		...
		for (var i = 0; i < firmaeducationdata.length; i++) {
			if (firmaeducationdata[i]['personelsayisi']==0) {
			  sureorani=0;
			  education_chart_data[i] = {"baslik":String(firmaeducationdata[i]['yil']),"egitimeayrilansureorani":sureorani};
			}else{
			  sureorani=parseFloat(firmaeducationdata[i]['educationtime']/firmaeducationdata[i]['personelsayisi']).toFixed(2);
			  education_chart_data[i] = {"baslik":String(firmaeducationdata[i]['yil']),"egitimeayrilansureorani":sureorani};
			}
		...
		columnTemplate.adapter.add("fill", function(fill, target) {
			for(var i = 0; i < education_chart_data.length; i++){
				if (target.dataItem.valueY==parseFloat(education_chart_data[i]['egitimeayrilansureorani']).toFixed(2)) {
